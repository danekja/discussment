...
public class ThreadListPanel extends Panel {
     ...
     */
    public ThreadListPanel(String id, IModel<List<Post>> threadListModel, IModel<Post> postModel, PostService postService, AccessControlService accessControlService) {
        super(id);
    ...
    protected void onInitialize() {
        ...
        add(new ListView<Post>("threadListView", threadListModel) {
            protected void populateItem(ListItem<Post> listItem) {
                listItem.add(new PostListPanel("postPanel", new PostWicketModel(listItem.getModel(), postService), postModel, postService, accessControlService));
            }
        });
    }
}
